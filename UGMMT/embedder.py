...
from property_handler import get_similar_patents_list
...
class Embedder(nn.Module):
    ...
    def forward(self, tuple_strings):
        ...
        recon_loss, batch_strings_recon_score, patent_loss = self.forward_decoder(tuple_strings, embedding)
    ...
    def forward_decoder(self, tuple_strings, embedding, usePatentsLoss=False):
        ...
        patent_loss = None
        if usePatentsLoss:
            output_batch = self.decoder_test(max_len=90, embedding=embedding, out_for_PatentsLoss=True)
            outputs_SMILES_list = [output_batch[j::len(tuple_strings)] for j, input in enumerate(tuple_strings)]
            outputs_SMILES_list = np.array([molecule_SMILES[0] for molecule_SMILES in outputs_SMILES_list])
            patent_strings_lists = [get_similar_patents_list(outputs_SMILES) for outputs_SMILES in outputs_SMILES_list]

            patent_loss_list = []
            for idx in range(10):
                patent_strings = []
                for patent_strings_list in patent_strings_lists:
                    patent_strings.append(patent_strings_list[idx])

                patent_tensors = [self.string2tensor(patent_string) for patent_string in patent_strings]
                patents_strings = nn.utils.rnn.pad_sequence(patent_tensors, batch_first=True, padding_value=self.dataset.c2i['<pad>'])

                strings_recon_score_final = strings_recon_score[:, :-1].contiguous().view(-1, strings_recon_score.size(-1))
                strings_patent_score_final = patents_strings[:, 1:].contiguous().view(-1)
                p2d = [0, 0, 0, strings_patent_score_final.shape[0] - strings_recon_score_final.shape[0]]
                strings_recon_score_final = F.pad(strings_recon_score_final, p2d, "constant", self.dataset.c2i['<pad>'])

                # strings reconstruction and reconstruction loss
                strings_recon_score = self.decoder_GRU_out_to_char_score(GRU_output)
                patent_loss_idx = torch.log(0.2 * F.cross_entropy(strings_recon_score_final, strings_patent_score_final,
                                                          ignore_index=self.dataset.c2i['<pad>']))
                patent_loss_list.append(patent_loss_idx)

            patent_loss = torch.mean(torch.stack(patent_loss_list))

        return recon_loss, strings_recon_score, patent_loss


    def decoder_test(self, max_len=100, embedding=None, temp=1.0, out_for_PatentsLoss=False):
        if out_for_PatentsLoss:
            final_output = self.decoder_test_helper(max_len, embedding, temp)
        else:
            with torch.no_grad():
                final_output = self.decoder_test_helper(max_len, embedding, temp)
    ...
    def decoder_test_helper(self, max_len=100, embedding=None, temp=1.0):
        # with torch.no_grad():
