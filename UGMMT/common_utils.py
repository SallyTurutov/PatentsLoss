...
# my files
from property_handler import property_calc, similarity_calc, smiles2fingerprint, is_valid_molecule, calc_PL_score
...
def process_results_file(res_file_path, args, valid_res_file_path, trainset):
    ...
    if valid_res_def.empty:
        valid_res_def = pd.DataFrame(columns=['input', 'output', args.property, 'sim', 'novel', 'PL'])
        valid_res_def = valid_res_def.append({'input':'invalid', 'output':'invalid', args.property:0, 'sim':0, 'novel':0, 'PL':0}, ignore_index=True)
    else:
        ...
        # add PL column for output molecules
        with mp.Pool(processes=mp.cpu_count()) as pool:
            valid_res_def['PL'] = pool.map(calc_PL_score, valid_res_def['output'].values)
        # valid_res_def['PL'] = np.vectorize(calc_PL_score)(valid_res_def['output'])
...
def valid_results_file_to_metrics(valid_res_file_path, args, num_source_mols):
    ...
    validity, diversity, novelty, property, similarity, SR, PL = [], [], [], [], [], [], []
    for retry_i in range(10):
        validity_mean, diversity_mean, novelty_mean, property_mean, similarity_mean, SR_mean, PL_mean = \
    ...
    validity_np, diversity_np, novelty_np, property_np, similarity_np, SR_np, PL_np = np.array(validity), \
        np.array(diversity), np.array(novelty), np.array(property), np.array(similarity), np.array(SR), np.array(PL)
...
def get_metics_for_sample(valid_df, args, how_many_samples, num_source_mols, seed):
    ...
    # calculate *** SR ***
    SR = len(valid_df[(valid_df['sim'] > args.SR_similarity) & (valid_df[args.property] > args.SR_property_val) &
                      (valid_df['PL'] < args.SR_PL_val) & (valid_df['novel'])]) / num_valid_mols

    return validity, diversity, valid_df['novel'].mean(), valid_df[args.property].mean(), valid_df['sim'].mean(), SR, valid_df['PL'].mean()