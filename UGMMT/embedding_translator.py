...
class Statistics(object):
    ...
    def update(self, loss, loss_GAN_AB, loss_GAN_BA, cycle_loss_A, cycle_loss_B, kl_loss_A, kl_loss_B, \
        ...
        if patent_loss is not None:
            self.patent_loss_epoch.append(patent_loss)
    ...
    def print(self):
        ...
        if any(self.patent_loss_epoch):
            Average_patents_loss = sum(self.patent_loss_epoch) / len(self.patent_loss_epoch)
            print('Average patents_loss = ' + str(Average_patents_loss.tolist()))


def save_checkpoint(current_criterion, best_criterion, T_AB, T_BA, model_A, model_B, epoch, args):
    ...
    if epoch is not None:
        saved_state = dict(best_criterion=best_criterion,
                           ...
                           model_A=model_A,
                           model_B=model_B)
        checkpoint_filename_path = args.checkpoints_folder + '/' + args.property + '/checkpoint_model' + str(epoch) + '.pth'
        torch.save(saved_state, checkpoint_filename_path)
