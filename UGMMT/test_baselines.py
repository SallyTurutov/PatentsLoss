...
def check_testset(args):
    ...
    model_results = [fn for fn in os.listdir('main_results')]
    model_results.sort()
    for model_result in model_results:
        results_file_path = 'main_results/' + str(model_result)
        valid_results_file_path = 'valid_results/' + str(model_result)

        print(' ')
        print('Loading results for model => ' + args.baseline)
        print('From file => ' + model_result)
        ...
        print('PL score => mean: ' + str(round(PL_mean, 3)) + '   std: ' + str(round(PL_std, 3)))
