...
from embedding_translator import Translator, Discriminator, weights_init_normal, LambdaLR, ReplayBuffer, Statistics, save_checkpoint, load_checkpoint
...
def parse_arguments():
    ...
    parser.add_argument('--num_retries', type=int, default=1, help='number of retries for each validation sample')
    parser.add_argument('--patentset_path', type=str, default='dataset/PL/SureChEMBL.txt', help='the patents dataset path')
    parser.add_argument('--patentsFp_path', type=str, default='dataset/PL/patentsFp.pkl', help='the patents-fp dictionary path')
    parser.add_argument('--SR_similarity', type=int, default=0.15, help='minimal similarity for success')
    parser.add_argument('--SR_property_val', type=int, default=0.7, help='minimal property value for success')
    parser.add_argument('--SR_PL_val', type=int, default=0.4, help='maximal PL value for success')
    ...
    parser.add_argument('--validation_freq', type=int, default=5, help='validate every n-th epoch')
    ...
    parser.add_argument('--patents_loss', default=True, action='store_true', help='use patents loss during training or not')
...
def train_iteration_T(real_A, real_B, model_A, model_B, T_AB, T_BA, D_A, D_B, loss_GAN, optimizer_T, fake_A_buffer, fake_B_buffer, args):
    ...
    if args.conditional:
        ...
        loss, _, patent_loss = model_B.forward_decoder(real_B, real_B_fp, args.patents_loss)
        loss.backward()
        optimizer_T.step()
    ...
    if args.use_EETN is False:                  # for ablation
        recon_loss_A, _, patent_loss = model_A.forward_decoder(real_A, real_A_emb, args.patents_loss)
        recon_loss_B, _, patent_loss = model_B.forward_decoder(real_B, real_B_emb, args.patents_loss)
    ...
    if args.cycle_loss:
        ...
        cycle_loss_A, _, patent_loss = model_A.forward_decoder(real_A, cycle_A_emb, args.patents_loss)
        cycle_loss_B, _, patent_loss = model_B.forward_decoder(real_B, cycle_B_emb, args.patents_loss)
    else:
    ...
    if args.cycle_loss and args.gan_loss is False and args.kl_loss is False:    # Main model: only cycle
        if patent_loss is not None:
            loss = 2 * cycle_loss_A + cycle_loss_B - patent_loss
        else:
    ...
    return loss, loss_GAN_AB, loss_GAN_BA, cycle_loss_A, cycle_loss_B, kl_loss_A, kl_loss_B, fake_A_emb, fake_B_emb, patent_loss
...
def validation(args, model_name, model_in, model_out, T, epoch, boundaries, random_seed_list, fig=None, ax=None):
    ...
    avg_similarity, avg_property, avg_SR, avg_validity, avg_novelty, avg_diversity, avg_PL =\
    ...
    return avg_similarity, avg_property, avg_SR, avg_validity, avg_novelty, avg_diversity, avg_PL
...
if __name__ == "__main__":
    ...
    if args.property is 'QED':
        ...
        print("property must be 'QED 'or 'DRD2'")
        exit()

    if args.conditional or args.no_pre_train or args.load_checkpoint:
    ...
    for epoch in range(args.epoch_init, args.epochs + 1):
        print(' ')
        print('epoch #' + str(epoch) + " (Started at " + str(datetime.datetime.now()) + ")")
        ...
        for i, (real_A, real_B) in enumerate(zip(A_train_loader, B_train_loader)):
            ...
            loss, loss_GAN_AB, loss_GAN_BA, cycle_loss_A, cycle_loss_B, kl_loss_A, kl_loss_B, fake_A_emb, fake_B_emb, patent_loss = \
            ...
            stats.update(loss, loss_GAN_AB, loss_GAN_BA, cycle_loss_A, cycle_loss_B, kl_loss_A, kl_loss_B,
                         loss_D_A, loss_D_A_real, loss_D_A_fake, loss_D_B, loss_D_B_real, loss_D_B_fake, patent_loss)
        ...
        if args.is_valid is True and (epoch == args.epoch_init or epoch % args.validation_freq == 0):
            if args.valid_direction is 'AB' or args.valid_direction is 'Both':
                if epoch == args.epoch_init:
                    fig_AB, ax_AB = plt.subplots()
                avg_similarity_AB, avg_property_AB, avg_SR_AB, avg_validity_AB, avg_novelty_AB, avg_diversity_AB, avg_PL_AB = \
            ...
            if args.valid_direction is 'BA' or args.valid_direction is 'Both':
                ...
                avg_similarity_BA, avg_property_BA, avg_SR_BA, avg_validity_BA, avg_novelty_BA, avg_diversity_BA, avg_PL_BA = \
            ...
            # save checkpoint
            best_criterion = save_checkpoint(current_criterion, best_criterion, T_AB, T_BA, model_A, model_B, epoch, args)
