...
class JTNNVAE(nn.Module):
    ...
    def forward(self, x_batch, beta, vocab):
        ...
        patent_SMILES_list = get_similar_patents_list(output_SMILES)

        patents_loss_list = []
        for patent_SMILES in patent_SMILES_list:
            mol_tree = MolTree(patent_SMILES)
            mol_tree.recover()
            mol_tree.assemble()
            tree_batch = [mol_tree]

            try:
                x_patents_batch, _, _ = tensorize(tree_batch, vocab, assm=False)
                patents_word_loss, patents_topo_loss, patents_word_acc, patents_topo_acc = self.decoder(x_patents_batch, z_tree_vecs)
                patents_loss = 3 * torch.log(0.2 * (patents_topo_loss + patents_word_loss))
                if patents_loss is not None:
                    patents_loss_list.append(patents_loss)
            except Exception as e:
                continue

        patents_loss = torch.mean(torch.stack(patents_loss_list)) if len(patents_loss_list) != 0 else None
        if patents_loss is None:
            loss = word_loss + topo_loss + assm_loss + beta * kl_div
            patents_loss = 0
        else:
            loss = word_loss + topo_loss + assm_loss + beta * kl_div - patents_loss
            patents_loss = patents_loss.item()

        return loss, kl_div.item(), word_acc, topo_acc, assm_acc, patents_loss
